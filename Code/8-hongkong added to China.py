...
for hsCode in hsCodes:
    ...
    while year <= 2019:
        ...
        #first delete the link between Hongkong and China
        sql0 =  "delete from %s" % tableName + " where (rtCode = 344 and ptCode = 156) or (rtCode = 156 and ptCode = 344) "
        cursor.execute(sql0)
        db.commit()
        sql1 = "select * from %s" % tableName + " where rtCode = 344 or ptCode = 344 and cmdCode=%d"%hsCode + " and period=%s"%str(year)
        cursor.execute(sql1)
        HkongData = cursor.fetchall()
